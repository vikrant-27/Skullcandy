...
def signup(request):
    if request.method=="POST":
        ...
        if password!=confirm_password:
            messages.warning(request,"Password is Not Matching")   #return HttpResponse("password incorrect")
        ...
        user.is_active=False        #to make the current user as non active by default 
        user.save()
        email_subject="Activate Your Account"
        message=render_to_string('activate.html',{
            'user':user,
            'domain':'127.0.0.1:8000',      #change it to your domain name or ip address and port number
